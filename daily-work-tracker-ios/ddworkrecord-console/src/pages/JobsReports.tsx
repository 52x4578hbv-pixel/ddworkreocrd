...
async function fetchXlsxExport(params: { startDate: string; endDate: string }, token: string | null) {
  ...
  // The route is actually under /admin/ in adminRoutes.ts
  const url = `${base}/api/v1/admin/reports/export/xlsx?${qs.toString()}`
...
export default function JobsReports() {
  ...
  const doExportXlsx = async () => {
    ...
    try {
      ...
      const filename = `job_reports_${startDate}_to_${endDate}.xlsx`
  ...
  return (
    <div style={{ fontFamily: 'system-ui', padding: 24, maxWidth: 980 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ margin: 0 }}>Jobs Reports</h1>
      ...
      <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
        <div style={{ padding: 12, border: '2px solid #0f172a', borderRadius: 12, background: '#fff' }}>
          <div style={{ color: '#64748b', fontWeight: 900, fontSize: 12 }}>Jobs workdays</div>
          <div style={{ marginTop: 6, fontWeight: 1000, fontSize: 24 }}>{totals.jobCount}</div>
        </div>

        <div style={{ padding: 12, border: '2px solid #0f172a', borderRadius: 12, background: '#fff' }}>
          <div style={{ color: '#64748b', fontWeight: 900, fontSize: 12 }}>Total job hours</div>
          <div style={{ marginTop: 6, fontWeight: 1000, fontSize: 24 }}>{totals.totalHours}</div>
        </div>

        <div style={{ padding: 12, border: '2px solid #0f172a', borderRadius: 12, background: '#fff' }}>
          <div style={{ color: '#64748b', fontWeight: 900, fontSize: 12 }}>Complete hours</div>
          <div style={{ marginTop: 6, fontWeight: 1000, fontSize: 24 }}>{totals.completeHours}</div>
        </div>

        <div style={{ padding: 12, border: '2px solid #0f172a', borderRadius: 12, background: '#fff' }}>
          <div style={{ color: '#64748b', fontWeight: 900, fontSize: 12 }}>Return required hours</div>
          <div style={{ marginTop: 6, fontWeight: 1000, fontSize: 24 }}>{totals.returnHours}</div>
        </div>
      </div>

      <div style={{ marginTop: 14, color: '#64748b', fontWeight: 800, fontSize: 12 }}>
        Note: local preview “job hours” use the workday total hours (template data). Real job-segment hours can be wired later if you have a separate computed field.
      </div>
